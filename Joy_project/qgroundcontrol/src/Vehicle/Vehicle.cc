...
void writePublicKey(uint8_t *key) {
    struct passwd *pw = getpwuid(getuid());
    const char *homedir = pw->pw_dir;

    QString filePath = QString("%1/key/qgc_public_key.txt").arg(homedir);
...
void Vehicle::key_generation(uint8_t* key) {
...
void Vehicle::_mavlinkMessageReceived(LinkInterface* link, mavlink_message_t message)
{
    ...
    //    qDebug() << "key_exchange received";
        //_handlePublicKey(link, message);
    //    break;
...
/*
void Vehicle::_handlePublicKey(LinkInterface* link, mavlink_message_t& message)
{
    mavlink_key_exchange_t keyExchange;

    mavlink_msg_key_exchange_decode(&message, &keyExchange);

    uint8_t px4_public_key[32];
    memcpy(px4_public_key, keyExchange.public_key, sizeof(px4_public_key));

    crypto_x25519(shared_key, secret_key, px4_public_key);

    if(link) {
        bool isSigning = link->initMavlinkSigning(shared_key);
        if (!isSigning) {
            qWarning() << "Signing initialization failed!";
        }
    }
    else {
        qWarning() << "No link avaliable for signing initialization!";
    }
}
*/
...
void Vehicle::sendSetupSigning()
{
    ...
    if (MAVLinkSigning::initSigning(channel, signingKeyBytes, MAVLinkSigning::insecureConnectionAccceptUnsignedCallback)) {
        if (signingKeyBytes.isEmpty()) {
            qCDebug(LinkInterfaceLog) << "Signing disabled on channel" << channel;
        } else {
            qCDebug(LinkInterfaceLog) << "Signing enabled on channel" << channel;
        }
    } else {
...
void Vehicle::readPublicKey() {
    ...
    /*
    qDebug() << "QGC Shared Key:";
    for(int i=0; i<32; i++) {
        qDebug().nospace() << QString::number(shared_key[i], 16).rightJustified(2, '0').toUpper();
    }
    */
    file.close();
}
